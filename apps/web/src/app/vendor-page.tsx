...
import { ApiError, vendorApi, type Course, type Vendor } from '../lib/api'
...
function AccessPanel({ vendor, accesses }: { vendor: Vendor; accesses: Array<{ id: string; learner_email: string; course_title: string; status: string }> }) {
  ...
  const queryClient = useQueryClient()
  const grant = useMutation({ mutationFn: () => vendorApi.grant(vendor.id, email, [courseId]), onSuccess: () => { setEmail(''); void queryClient.invalidateQueries({ queryKey: ['access', vendor.id] }) } })
